...
//+------------------------------------------------------------------+
void remove_elements(string prefix)
  {
...
double Account_Balance()
   {
   if(AccountBalance()==0.0) {return 0.01;}
...
//| Set event timer                                                  |
//+------------------------------------------------------------------+
bool set_event_timer(int refresh_rate_)
   {
   ResetLastError();
   int xtimer;
   // checking connection to broker
   for(xtimer=1;xtimer<5;xtimer++)
      {
      if(xtimer==4)
         {
         // this means the loop has exceeded 3 attempts. Return false
         MessageBox("Setting Event timer failed. Please check whther your system's"
                     " RAM and CPU utilization is not 100%"
                     ,"AutoBot",MB_ICONINFORMATION);
         return false;
         }
      
      Print(__FUNCTION__," :Setting Event Timer.. Attempt ",xtimer);
      if(EventSetTimer(refresh_rate_)){Print(__FUNCTION__," :Event timer set");break;}
      else{Print(__FUNCTION__," :Event Set Fail Error: ",GetLastError());}
...
//+------------------------------------------------------------------+
//| check market condictions                                         |
//+------------------------------------------------------------------+ 
bool check_market_open(string symbol_to_check,datetime &last_server_time_)
   {
   if(!IsConnected())
      {
      return false; // If not connected ignore iteration
      }   
   
   if(last_server_time_==0)
      {
      Print(__FUNCTION__," :Setting_last Server Time");
      if(!SymbolSelect(symbol_to_check,true))
         {
         Print(__FUNCTION__," :Symbol select fail. Symbol: ",symbol_to_check);
         return false; // exit if symbols fails
         }
      last_server_time_ = TimeCurrent();
      return false; // first iteration is ignored
      }
   ...
   if(last_server_time_==TimeCurrent())
      {
      //Print(__FUNCTION__," :Market Closed");
      return false; // market is closed
      }
      
   last_server_time_ = TimeCurrent(); // set the current time
   return true;
   }
...
//| check auto trading                                               |
//+------------------------------------------------------------------+
bool check_auto_trading()
   {
   if(IsExpertEnabled() && IsTradeAllowed())
      {
      Print(__FUNCTION__," :Auto Trading Settings ok");
      return true;
      }
   else
      {
      Print(__FUNCTION__," :Auto Trading settings not enabled.");
      MessageBox("AutoTrading is not enabled."
                  " Please make sure the following.\n\n"
                  "1. AutoTrading button is pressed in MetaTrader.\n\n"
                  "2. 'Allow live trading' is enabled in Expert Advisors > Properties > Common.\n"
                  ,"AutoBot",MB_ICONINFORMATION);      
      return false;
      }
   }
...
bool check_version_expiration(datetime version_exp_)
   {
   if(version_exp_<Time[0]){return true;}
   else{return false;}
   }
